## Sammy's Nest Mobile and Web App
This is a personal mobile and web application built by Rice University undergrads during moments of peace and quiet.While this project is open-source and anyone may borrow code or inspiration from Sammy's Nest, we would like for credit to be given for the production and launch of Sammy's Nest.

## Goal of Sammy's Nest
Sammy's Nest aims to be an all-in-one hub for Rice University students to find and engage in campus events related to their major, minor, degree, or extra-curricular interests. Currently, campus events are isolated in private mailing lists, 
...
Sammy's Nest serves to solve all of these problems through one intuitive, easy-to-use app. By scraping event data from Rice University department websites and campus clubs, we aim to provide a hub for students to view all campus events occuring on any given day. Students can then filter events by degree, club, etc. as well as sign up to be notified when an event they're interested in pops up. 

To make Sammy's Nest more accessible, we've included many language options as well as design tweaks that make the app more intutiive to use based on a student's language preference, location, time-zone, etc. 
...
## Set-up Sammy's Nest for Personal Usage
Please strictly adhere to the following instructions when setting up Sammy's Nest for personal tinkering.

1. Clone the Repository
`git clone https://github.com/mikemao27/Sammy-Events.git`
`cd Sammy-Events`
